...
export default function CompleteScreen() {
  ...
  const [status, setStatus] = useState<
    "creating" | "generating" | "done" | "error"
  >("creating");
  const [error, setError] = useState<string | null>(null);

  const createProfile = useAction(api.users.createProfileWithEmbeding);

  const aimPulse = useSharedValue(1);

  useEffect(() => {
    aimPulse.value = withRepeat(
  ...
  const pulseStyle = useAnimatedStyle(() => ({
    transform: [{ scale: aimPulse.value }],
  }));
    ...
    const createUserProfile = async () => {
      ...
      try {
        ...
        const maxDistance = params.maxDistance
          ? parseInt(params.maxDistance as string, 10)
          : undefined;
          ...
          >
            <Animated.View style={[styles.pulseContainer, pulseStyle]}>
              ...
              >
                <Ionicons
                  name={
                    status === "creating" ? "document-text-outline" : "sparkles"
                  }
                  size={48}
                  color={colors.primary}
                />
