...
import { Countdown, EmptyState } from "@/components/ui";
...
export default function matches() {
  ...
  ) as DailyPicksData | undefined | null;

  const generateDailyPicks = useAction(api.matches.generateDailyPicks);
  const actionOnPick = useAction(api.matches.actOnDailyPick);
  ...
  // generate daily picks if needed
  useEffect(() => {
    const generateIfneeded = async () => {
      if (dailyPicks === null && !isGenerating && currentUser?._id) {
  ...
  const pendingPicks =
    dailyPicks?.picks.filter((pick) => pick.status === "pending") ?? [];
  const allReviewed =
   true
  ...
  const handleLikeForPick = async (pick: DailyPickWithUser) => {
    ...
    try {
      const result = await actionOnPick({
        action: "like",
