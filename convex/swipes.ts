...
export const getSwipeInternal = internalQuery({
...
export const getLikesReceived = query({
  ...
  handler: async (ctx, args) => {
    const likeReceived = await ctx.db
      .query("swipes")
    ...
    for (const like of likeReceived) {
      ...
      if (!ourSwipe) {
        ...
        if (user) {
          const userWithPhoto = await withResolvedPhotos(ctx, user);
          pendingLikes.push(userWithPhoto);
        }
      }
    }
...
export const getSwipeFeed = query({
  ...
  handler: async (ctx, args) => {
    ...
    for await (const user of ctx.db.query("users")) {
      ...
      if (!areUserCmpatible(currentUser, user)) continue;
